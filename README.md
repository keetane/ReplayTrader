...
画面右上の「約定履歴CSV」から、証券会社などから取得した取引履歴CSVを読み込めます。隣の目のアイコンで実約定の表示/非表示を切り替えられます。Shift-JISとUTF-8に対応しています。

表示対象は、`状況=約定`、`約定数量>0`、`約定単価`と`約定代金`が有効な行だけです。未約定、取消、約定数量0の注文は除外されます。実約定履歴は選択中の銘柄に対応するものだけが、リプレイの進行に合わせてチャートと右側の一覧に表示されます。

実約定履歴は読み取り専用で、仮想口座の残高、建玉、損益には影響しません。取引履歴CSVに約定日時がない場合は、CSVの注文日時を表示時刻として使用します。
